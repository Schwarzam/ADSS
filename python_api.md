...
You may print the database metadata to see what is available:


```python
metadata = cl.get_database_metadata()

cl.pretty_print_metadata()
```


```bash
public.delve_dr2: id, ra, dec, mag_auto_g, mag_auto_r, z_p...
splus.splus_dr4: id, ra, dec, a, b, background, backgroun...
```
...
"""
select * 
from upload.my_table 
where cone(ra, dec, 150.0, 2.0, 0.1)
"""
)
```
...
tab = cl.query_and_wait(
    ...
    """,
    mode="astroql", # astroql, adql or sql
    file=None, # dataframe
    table_name=None,
)
# Print the dataframe
...
Then handle the bytes, fits or image. Example:
...
You can listen to live alerts directly from the client:

```python
for alert in cl.listen_alerts(categories=["transient", "agn"]):
    print(alert)
```

To inspect the available categories:

```python
categories = cl.get_alert_categories()
print(categories)
```

You can also replay retained alerts and stop when the replay is complete:

```python
for alert in cl.listen_alerts(
    categories="all",
    replay="earliest",
    limit=100,
    follow=False,
...
Use `Ctrl+C` to stop a live stream.
